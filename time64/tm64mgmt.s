 TITLE '/home/mike/repos/crent370/time64/tm64mgmt.c'
         COPY  PDPTOP
         CSECT
...
* Program text area
         DS    0F
         EJECT
* external function 'mgmtime64' prologue
* frame base=88, local stack=8, call args=16
&FUNC    SETC  'mgmtime64'
TM64MGMT PDPPRLG CINDEX=0,FRAME=112,BASER=12,ENTRY=YES
         B     @@FEN0
         LTORG
@@FEN0   EQU   *
         DROP  12
...
@@PG0    EQU   *
         LR    11,1
         L     10,=A(@@PGT0)
* Function 'mgmtime64' code
         LA    1,88(,13)
         L     15,=V(@@CRTGET)
         BALR  14,15
         LR    2,15 ==> crt
         A     2,=F'296' ==> tms
         MVC   88(4,13),0(11) ==> mtimer
         MVC   92(4,13),=F'1000'
         LA    3,104(,13)
         ST    3,96(13)
         LA    1,88(,13)
         ...
         L     15,=V(TM64GMTR)
         BALR  14,15
* Function 'mgmtime64' epilogue
         PDPEPIL
* Function 'mgmtime64' literal pool
         DS    0D
         LTORG
* Function 'mgmtime64' page table
@@PGT0   DS    0F
         DC    A(@@PG0)
         END
