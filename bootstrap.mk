# bootstrap.mk — Modular bootstrap and doctor targets for crent370
#
...
REQUIRED_VARS := MVS_HOST MVS_PORT MVS_USER MVS_PASS \
                 CRENT_PUNCH CRENT_SYSLMOD
...
_doc_connect:
	...
	@if [ -z "$(MVS_HOST)" ] || [ -z "$(MVS_USER)" ]; then \
		printf "$(_RED)[FAIL]$(_RST) %s\n" "MVS_HOST or MVS_USER not set"; \
		exit 0; \
	fi; \
	HTTP=$$(curl $(CURL_OPTS) -o /dev/null -w '%{http_code}' \
		-u "$(AUTH)" \
...
_doc_datasets:
	$(call _hdr,"Datasets")
	@if [ -z "$(MVS_HOST)" ] || [ -z "$(MVS_USER)" ]; then exit 0; fi; \
	for dsvar in CRENT_PUNCH CRENT_SYSLMOD; do \
...
run-mvs:
	...
	if docker inspect $(MVS_CONTAINER) >/dev/null 2>&1; then \
		...
		docker run -d --name $(MVS_CONTAINER) --network $(DOCKER_NETWORK) \
			...
			$(MVS_IMAGE); \
		echo ""; \
		echo "Set MVS_HOST=$(MVS_CONTAINER) in your .env to connect to this container."; \
	fi; \
	if [ -f /.dockerenv ]; then \
		docker network connect $(DOCKER_NETWORK) $$(hostname) 2>/dev/null || true; \
	fi
